...
export async function POST(request: Request) {
  ...
  if (!url || !token) {
    return NextResponse.json(
      { ok: false, message: "La agenda de evaluaciones todavía no está habilitada en este entorno." },
      { status: 503 },
    );
  }
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(body.date || "")) {
    return NextResponse.json({ ok: false, message: "Fecha inválida." }, { status: 400 });
  }
  ...
  try {
    const upstream = await fetch(url, {
      ...
      body: JSON.stringify({ service: MEDSPORT_EVALUATION_SERVICE, date: body.date }),
      cache: "no-store",
    ...
    if (!upstream.ok) {
      return NextResponse.json(
        { ok: false, message: data?.message || "ATHENAS no pudo consultar la disponibilidad de evaluaciones." },
    ...
    return NextResponse.json(
      { ok: false, message: "No fue posible conectar con ATHENAS para consultar evaluaciones." },
      { status: 502 },
    );
  } finally {
    clearTimeout(timer);
